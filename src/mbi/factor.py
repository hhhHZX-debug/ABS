...
class Factor:
    ...
    def expand(self, domain):   # 一个势函数拓展另一个势函数
    ...
    def sum(self, attrs = None):    # 数据中排除属性attrs
        if attrs is None:
    ...
    def mul_vec(self, other):
        ...
        #print(self.values.max(), other.values.max(), self.domain, other.domain)
        newdom = self.domain.merge(other.domain)
        print('newdom', newdom.attrs)
        print('self domain', self.domain.attrs)
        print('other domain', other.domain.attrs)
        factor1 = self.expand(newdom)
        print('self', factor1.domain.attrs, factor1.values)
        factor2 = other.expand(newdom)
        print('other', factor2.domain.attrs, factor2.values)
        print('final dis', factor1.values * factor2.values)
        return Factor(newdom, factor1.values * factor2.values)
