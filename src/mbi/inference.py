...
class FactoredInference:
    ...
    #def estimate(self, measurements, total=None, engine='IG', callback=None, options={}):
        """ 
    ...
    def mirror_descent(self, measurements, total=None, stepsize=None, callback=None):
        ...
        self._setup(measurements, total)  # 构造图模型
        model = self.model      # 指向self.model的地址
        cliques, theta = model.cliques, model.potentials  # 最大团，初始化团分布为0分布
        mu = model.belief_propagation(theta)  # 信念传播生成边缘分布 生成无约束的最大熵分布
        ans = self._marginal_loss(mu)  # 最大团的分布为输入  计算每个最大团 边缘分布与噪音分布的差异 以及此次的梯度  返回 总损失值与对应梯度
        if ans[0] == 0:
            return ans[0]
        ...
        for t in range(1, self.iters + 1):
            ...
            alpha = stepsize(t)  # alpha为步长
            # print('curr_loss', curr_loss)
            for i in range(25):
                theta = omega - alpha * dL  # theta  更新此轮的向量= 前一轮向量-步长*梯度
                mu = model.belief_propagation(theta)  # mu为信念传播后的边缘分布集合
                ans = self._marginal_loss(mu)  # 计算新损失与新梯度
                if nols or curr_loss - ans[0] >= 0.5 * alpha * dL.dot(nu - mu):     # 判定当前的损失减小两是否大于移动步长 如果步长太小就需要放大步长
                    break
                alpha *= 0.5  # 步长变为原来的一半
    ...
    def _marginal_loss(self, marginals, metric=None):
        ...
        for cl in marginals:
            ...
            for Q, y, noise, proj in self.groups[cl]:  # groups[cl]包含 cl的（部分）子集
                c = 1.0 / noise
                mu2 = mu.project(proj)
                x = mu2.datavector()    # 图模型投影到属性proj上的分布向量
                diff = c * (Q @ x - y)  # diff等于 (噪音分布与 图模型分布的距离)/ sigma
                if metric == 'L1':
                    loss += abs(diff).sum()
                    sign = diff.sign() if hasattr(diff, 'sign') else np.sign(diff)
                    grad = c * (Q.T @ sign)
                else:
                    loss += 0.5 * (diff @ diff)  # l2范数的平方*0.5有助于收敛     @等价于点乘
                    grad = (Q.T @ diff)
    ...
    def _setup(self, measurements, total):
        ...
        for Q, y, noise, proj in measurements:
            ...
            m = (Q, y, noise, proj)
            for cl in sorted(cliques, key=model.domain.size):   #根据域从小到大对最大团排序
                # (Q, y, noise, proj) tuple
                if set(proj) <= set(cl):
                    self.groups[cl].append(m)       #在最大团cl中添加边缘m
                    break
